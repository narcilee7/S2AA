...
// TODO: kernelImagePath and rootfsPath should come from FactoryConfig or image registry.
func newMicroVMSandbox(
	...
	// TODO: launch Firecracker process and configure the VM.
	// For now we leave vmPID = 0 and agent = nil; the sandbox skeleton is in place.
	return s, nil
}

// startVM is a placeholder for the full Firecracker bootstrap sequence.
func (s *microvmSandbox) startVM(ctx context.Context) error {
	// 1. Ensure firecracker binary exists.
	if _, err := exec.LookPath("firecracker"); err != nil {
		return fmt.Errorf("firecracker binary not found: %w", err)
	}

	// 2. Prepare kernel and rootfs paths (must be provided by operator).
	...
	// 3. Launch firecracker with API socket.
	...
	// 5. Configure VM via HTTP over Unix socket.
	...
	// 6. Dial guest agent via vsock with retries.
	s.agent = newAgentClientVSOCK(s.guestCID, s.guestPort)
	for i := 0; i < 60; i++ {
	...
	// 7. Start network proxy for egress filtering.
...
func (s *microvmSandbox) fcPut(ctx context.Context, path string, body interface{}) error {
	...
	// Use curl for simplicity; can be replaced by a unix-socket HTTP client later.
...
func (s *microvmSandbox) Execute(ctx context.Context, cmd Command) (*Result, error) {
	...
	return &Result{
		...
		SandboxID:     s.id,
		SecurityLevel: LevelIsolated, // Represented as L3 for now
...
// ExecuteStreaming is currently delegated to Execute with buffered callbacks.
...
func (s *microvmSandbox) Cancel(commandID string) error {
	...
	defer cancel()
	_, err := s.agent.Cancel(ctx, AgentCancelRequest{CommandID: commandID})
	return err
}
...
func (s *microvmSandbox) Cleanup() error {
	...
	if s.vmPID > 0 {
		...
		// Fallback: kill the firecracker process after a short grace period.
...
func (fs *microvmFilesystem) ReadFile(ctx context.Context, path string) ([]byte, error) {
	...
	return fs.agent.ReadFile(ctx, path)
}
...
func (fs *microvmFilesystem) WriteFile(ctx context.Context, path string, data []byte, perm os.FileMode) error {
	...
	_, err := fs.agent.WriteFile(ctx, AgentWriteFileRequest{
		Path: path,
		Data: data,
		Mode: uint32(perm),
	})
...
func (fs *microvmFilesystem) DeleteFile(ctx context.Context, path string) error {
	...
	_, err := fs.agent.DeleteFile(ctx, path)
	return err
}

func (fs *microvmFilesystem) MkdirAll(ctx context.Context, path string, perm os.FileMode) error {
	// Guest agent does not expose a dedicated mkdir endpoint yet;
	// we emulate it by writing an empty file to force parent creation.
	if err := fs.ensureAgent(); err != nil {
		return err
	}
	_, err := fs.agent.WriteFile(ctx, AgentWriteFileRequest{
		Path: filepath.Join(path, ".mkdir"),
		Data: []byte{},
		Mode: uint32(perm),
	})
	if err != nil {
	...
	_, _ = fs.agent.DeleteFile(ctx, filepath.Join(path, ".mkdir"))
	return nil
}

func (fs *microvmFilesystem) Stat(ctx context.Context, path string) (*FileInfo, error) {
	entries, err := fs.ListFiles(ctx, filepath.Dir(path))
	if err != nil {
		return nil, err
	}
...
func (pf *microvmPortForwarder) ExposePort(ctx context.Context, containerPort int) (string, func(), error) {
	...
	resp, err := pf.agent.ExposePort(ctx, AgentPortRequest{ContainerPort: int32(containerPort)})
	if err != nil {
		return "", nil, err
	}
